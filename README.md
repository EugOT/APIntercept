...
- Split local development can set `NEXT_PUBLIC_INTERCEPTOR_WS_URL=ws://localhost:3001`; production can leave it empty when the reverse proxy routes `/ws` and `/browser/*` on the web origin.
